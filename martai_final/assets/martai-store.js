(function(){
  ...
  async function saveTableDB(db){
    ...
    if(settingsDirty&&isMainAdminSession()){
      ...
      r=await client.from('mart_settings').update({bank_weekend_days:s.bankWeekendDays||[0,6],bank_holidays:s.bankHolidays||[],updated_at:now()}).eq('id',true);
      if(r.error&&!/bank_weekend_days|bank_holidays|schema cache|column/i.test(String(r.error.message||'')))throw r.error;
  ...
  if('serviceWorker' in navigator){let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});window.addEventListener('load',()=>{navigator.serviceWorker.register('sw.js?v=63',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{})})}
